...
keep this repository portable: no host paths, domains, runtime choice, OIDC
provider brand, or VLM provider brand belongs here.
...
The UI refers to a generic OIDC identity provider and an OpenAI-compatible
vision inference provider. Coordinate API-client regeneration with backend
contract changes.

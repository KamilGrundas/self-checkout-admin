...
function IntegrationKeys() {
  ...
  const integration = useQuery({
    queryKey: ["integration-key", "vision-inference"],
    queryFn: () => api<IntegrationKey>("integrations/vision-inference"),
  })
  const save = useMutation({
    mutationFn: () =>
      api<IntegrationKey>("integrations/vision-inference", {
        method: "PUT",
        body: JSON.stringify({
          endpoint_url: integration.data?.endpoint_url,
          api_key: token || null,
          clear_api_key: clear,
        }),
      }),
    onSuccess: (data) => {
      setToken("")
      setClear(false)
      queryClient.setQueryData(["integration-key", "vision-inference"], data)
      void queryClient.invalidateQueries({ queryKey: ["autolabel-settings"] })
      void queryClient.invalidateQueries({ queryKey: ["autolabel-models"] })
    },
  })
  return (
    <section className="space-y-4 border-t pt-6">
      <h2 className="text-xl font-semibold">{t("apiKeysIntegrations")}</h2>
      <h3 className="font-semibold">{t("visionInferenceProvider")}</h3>
      <p>{t("apiKeyExternalHelp")}</p>
      <p>{integration.data?.endpoint_url || t("apiKeySaveEndpointFirst")}</p>
      <label className="block" htmlFor="vision-inference-api-key">
        {t("autolabelApiKey")}
      </label>
      <Input
        id="vision-inference-api-key"
        type="password"
        autoComplete="new-password"
        value={token}
        disabled={clear || !integration.data?.endpoint_url}
        placeholder={
      ...
      <label
        ...
        htmlFor="vision-inference-clear-key"
      >
        <input
          id="vision-inference-clear-key"
          type="checkbox"
          checked={clear}
          onChange={(event) => {
      ...
      <Button
        disabled={save.isPending || !integration.data?.endpoint_url}
        onClick={() => save.mutate()}
      >
        {t("saveConfiguration")}
      </Button>
      {save.isSuccess && <p role="status">{t("autolabelSettingsSaved")}</p>}
      {(integration.isError || save.isError) && (
        <p role="alert">{t("apiKeyError")}</p>
      )}
    </section>
  )
